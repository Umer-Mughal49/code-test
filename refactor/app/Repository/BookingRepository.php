...
//            $jobs['total'] = $totaljobs;
    ...
    public function store($user, $data)
    {
        ...
        if ($user->user_type == env('CUSTOMER_ROLE_ID')) {
            ...
            $response['status'] = $job ? 'success' : 'error';
            $response['id'] = $job->id;
            $data['job_for'] = array();
            if ($job->gender != null) {
                if ($job->gender == 'male') {
...
//            $this->sendNotificationToSuitableTranslators($job->id, $data, '*');// send Push for New job posting
    ...
    public function jobEnd($post_data = array())
    {
        ...
        $job_detail = Job::query()->with('translatorJobRel')->find($jobid);
...
//        $jobs = TeHelper::convertJobIdsInObjs($job_ids);
    ...
    public function updateJob($id, $data, $cuser)
    {
        ...
        if ($job->due <= Carbon::now()) {
            ...
            $job->save();
            if ($changeDue['dateChanged']) $this->notificationRepository->sendChangedDateNotification($job, $old_time);
            if ($changeTranslator['translatorChanged']) $this->notificationRepository->sendChangedTranslatorNotification($job, $current_translator, $changeTranslator['new_translator']);
            if ($langChanged) $this->notificationRepository->sendChangedLangNotification($job, $old_lang);
        }
    }
...
//        if (in_array($data['status'], ['pending', 'assigned']) && date('Y-m-d H:i:s') <= $job->due) {
        ...
        if ($data['status'] == 'pending') {
            ...
            $this->notificationRepository->sendNotificationTranslator($job, $job_data, '*');   // send Push all sutiable translators
...
//        if (in_array($data['status'], ['withdrawnbefore24', 'withdrawafter24', 'timedout', 'assigned'])) {
        ...
        if ($data['status'] == 'assigned' && $changedTranslator) {
            ...
            $this->notificationRepository->sendSessionStartRemindNotification($user, $job, $language, $job->due, $job->duration);
            $this->notificationRepository->sendSessionStartRemindNotification($translator, $job, $language, $job->due, $job->duration);
            return true;
        } else {
        ...
        if (!Job::isTranslatorAlreadyBooked($job_id, $cuser->id, $job->due)) {
            if ($job->status == 'pending' && Job::insertTranslatorJobRel($cuser->id, $job_id)) {
                ...
                if ($this->isNeedToSendPush($user->id)) {
                    ...
                    $this->notificationRepository->sendPushNotificationToSpecificUsers($users_array, $job_id, $data, $msg_text, $this->isNeedToDelayPush($user->id));
                }
        ...
        if ($cuser->is('customer')) {
            ...
            if ($translator) {
                ...
                if ($this->isNeedToSendPush($translator->id)) {
                    ...
                    $this->notificationRepository->sendPushNotificationToSpecificUsers($users_array, $job_id, $data, $msg_text, $this->isNeedToDelayPush($translator->id));// send Session Cancel Push to Translaotor
                }
            }
        } else {
            if ($job->due->diffInHours(Carbon::now()) > 24) {
                ...
                if ($customer) {
                    ...
                    if ($this->isNeedToSendPush($customer->id)) {
                        ...
                        $this->notificationRepository->sendPushNotificationToSpecificUsers($users_array, $job_id, $data, $msg_text, $this->isNeedToDelayPush($customer->id));     // send Session Cancel Push to customer
                    }
                }
                $job->status = 'pending';
                $job->created_at = date('Y-m-d H:i:s');
...
//                Event::fire(new JobWasCanceled($job));
                ...
                $this->notificationRepository->sendNotificationTranslator($job, $data, $translator->id);   // send Push all sutiable translators
                $response['status'] = 'success';
            } else {
                $response['status'] = 'fail';
...
//        $this->logger->addInfo('USER #' . Auth::user()->id . ' reopen booking #: ' . $jobid);
        ...
        if (isset($affectedRows)) {
            $this->notificationRepository->sendNotificationByAdminCancelJob($new_jobid);
